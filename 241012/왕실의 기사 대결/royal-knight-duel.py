...
def is_movable(i, d):
    ...
    if not alive[i]:
        raise ValueError(f'Knight {i} is dead.')
...
def movable_knight(i, d):
    # 명령을 받은 기사는 아니고, 밀려나는 기사로 가정.
    global knight_dict, board, knights, alive
...
for ii, command in enumerate(commands):
    ...
    if not is_movable(i, d) or not movable_knight(i, d):
        continue
    move_knight(i, d, True)
    print_2d_graph(f'[Turn {ii}. i: {i}, d: {d}], knights', knights)
    print_dict(f'[Turn {ii}. i: {i}, d: {d}], knights_dict', knight_dict)

for _k, _v in knight_dict.items():
    r, c, h, w, k = _v
    if not alive[_k] or k <= 0:
        continue
    # if k != initial_info[_k-1][-1]:
    #     print(f'knight: {_k}, k: {k}, init: {initial_info[_k-1][-1]}')
